...
namespace zone
{
	...
	namespace
	{
			...
			fail(const string& m)
		{
			...
			MessageBoxA(nullptr, m.c_str(), "ZW3", MB_ICONERROR | MB_OK);
	...
	namespace
	{
		...
		// copy under zone/zw3/x86.
		//
		// Note that this means zw3 will turn up when we enumerate
		...
		const fs::path converted_root("zone/zw3/x86");
		const string converted_name("zw3");
		const fs::path iw4x_main_root("main/iw4x/x86");
		const fs::path zw3_main_root("main/zw3/x86");
		const fs::path iw4x_zone_root("zone/iw4x/x86");

		// The loose ZW3 files are a little different. The game loads these
	...
	namespace
	{
		...
		// arranging ZW3 files, though some of the fastfiles in it are
		// Steam's.
		//
	...
	namespace
	{
		// There is first the old ZW3 directory next to the game. What used
			...
			backup_language(const fs::path& b)
		{
			...
			{
				fail("\"{}\" holds the fastfiles an older version of ZW3 replaced, and "
		...
		void
			plan_iw4x_tree(plan& p, const fs::path& from, const fs::path& to)
		{
			if (!fs::is_directory(from))
				return;

			for (const fs::directory_entry& e : fs::recursive_directory_iterator(from))
			{
				if (!e.is_regular_file())
					continue;

				const fs::path relative(e.path().lexically_relative(from));
				const fs::path destination(to / relative);

				// Preserve an existing ZW3 copy when it is the same file. A size
				// difference always needs refreshing; for equal-sized files the
				// source timestamp tells us whether IW4x supplied a newer version.
				if (fs::exists(destination) &&
					fs::file_size(destination) == e.file_size() &&
					fs::last_write_time(destination) >= e.last_write_time())
				{
					continue;
				}

				p.push_back({ action::copy, e.path(), destination });
			}
		}

		void
			plan_iw4x_x86(plan& p)
		{
			// Recent IW4x releases already ship the x86 files ZW3 needs. Mirror
			// those trees into ZW3's independent search paths rather than trying
			// to run the x64 converter over them or making ZW3 load from IW4x.
			plan_iw4x_tree(p, iw4x_main_root, zw3_main_root);
			plan_iw4x_tree(p, iw4x_zone_root, converted_root);
		}
			...
			plan_recovery(plan& p)
		{
			...
			for (const fs::directory_entry& e : fs::directory_iterator(zone_root))
			{
				...
				fs::path cd(converted_root / g);  // Where zw3 reads it from.
		...
		const phase_type phases[] = {
		  {"moving ZW3's own files under \"" BASEGAME "\"", &plan_basegame},
		  {"copying IW4x's x86 files for ZW3",                &plan_iw4x_x86},
		  {"putting the stock layout back",                  &plan_recovery},
		  {"taking over ZW3's fastfiles",                   &plan_adoption},
	...
	namespace
	{
		...
		const char* const window_class("ZW3ZoneConvert");
	...
	namespace
	{
		...
		vector<string>
			spawn(const vector<const step*>& ss, progress& pg, size_t already_done)
		{
			...
			execute(const plan& p, progress& pg)
		{
			...
			for (const step& s : p)
			{
				...
				if (s.act == action::copy)
					pg.running({ s.from.filename().generic_string() });
			...
			//
			return ss.empty() ? vector<string>() : spawn(ss, pg, done);
		}
	}
		...
		for (const phase_type& ph : phases)
		{
			...
			const size_t copies(static_cast<size_t> (
				ranges::count_if(p,
					[](const step& s)
					{
						return s.act == action::copy;
					})));
			const size_t visible_work(n + copies);
			...
			{
				window_progress pg("ZW3 - Preparing x86 files", visible_work);
				f = execute(p, pg);
			}

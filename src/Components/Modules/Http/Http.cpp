...
{
	mg_mgr Http::Mgr;
	Http::ClientDownload Http::CLDownload;
	std::vector<std::shared_ptr<Http::ScriptDownload>> Http::ScriptDownloads;

	std::thread Http::ServerThread;
	bool Http::Terminate;
    bool Http::ServerRunning;

#pragma region Client

	void Http::InitiateMapDownload(const std::string& map, bool needPassword)
	{
		Http::InitiateClientDownload(map, needPassword, true);
	}

	void Http::InitiateClientDownload(const std::string& mod, bool needPassword, bool map)
	{
		if (Http::CLDownload.running) return;
		...
		if (needPassword)
		{
			...
			Http::CLDownload.hashedPassword = Utils::String::DumpHex(Utils::Cryptography::SHA256::Compute(pass), "");
		}

		Http::CLDownload.running = true;
		Http::CLDownload.isMap = map;
		Http::CLDownload.mod = mod;
		Http::CLDownload.terminateThread = false;
		Http::CLDownload.totalBytes = 0;
		Http::CLDownload.lastTimeStamp = 0;
		Http::CLDownload.downBytes = 0;
		Http::CLDownload.timeStampBytes = 0;
		Http::CLDownload.isPrivate = needPassword;
		Http::CLDownload.target = Party::Target();
		Http::CLDownload.thread = std::thread(Http::ModDownloader, &Http::CLDownload);
	}

	bool Http::ParseModList(ClientDownload* download, const std::string& list)
	{
		...
		for (auto& file : listData.array_items())
		{
			...
			Http::ClientHttp::File fileEntry;
	...
	void Http::DownloadHandler(mg_connection *nc, int ev, void* ev_data)
	{
		http_message* hm = reinterpret_cast<http_message*>(ev_data);
		Http::FileDownload* fDownload = reinterpret_cast<Http::FileDownload*>(nc->mgr->user_data);
		...
		if (ev == MG_EV_RECV)
		{
			...
			Http::DownloadProgress(fDownload, bytes);
		}
	...
	bool Http::DownloadFile(ClientDownload* download, unsigned int index)
	{
		...
		mg_connect_http(&download->mgr, Http::DownloadHandler, url.data(), nullptr, nullptr);
		...
		webIO.setProgressCallback([&fDownload, &webIO](size_t bytes, size_t)
		{
			...
			Http::DownloadProgress(&fDownload, bytes - fDownload.receivedBytes);
		});
	...
	void Http::ModDownloader(ClientDownload* download)
	{
		if (!download) download = &Http::CLDownload;
		...
		if (!Http::ParseModList(download, list))
		{
			if (download->terminateThread) return;
		...
		for (unsigned int i = 0; i < download->files.size(); ++i)
		{
			...
			if (!Http::DownloadFile(download, i))
			{
				if (download->terminateThread) return;
...
#pragma region Server
	...
	bool Http::IsClient(mg_connection *nc)
	{
		return (Http::GetClient(nc) != nullptr);
	}

	Game::client_t* Http::GetClient(mg_connection *nc)
	{
	...
	void Http::DownloadProgress(FileDownload* fDownload, size_t bytes)
	{
	...
	bool Http::VerifyPassword(mg_connection *nc, http_message* message)
	{
	...
	void Http::Forbid(mg_connection *nc)
	{
		mg_printf(nc, "HTTP/1.1 403 Forbidden\r\n"
			"Content-Type: text/html\r\n"
			"Connection: close\r\n"
	...
	void Http::ServerlistHandler(mg_connection* nc, int ev, void* /*ev_data*/)
	{
	...
	void Http::MapHandler(mg_connection *nc, int ev, void* ev_data)
	{
		// Only handle http requests
		if (ev != MG_EV_HTTP_REQUEST) return;

		if (!Http::VerifyPassword(nc, reinterpret_cast<http_message*>(ev_data))) return;
	...
	void Http::ListHandler(mg_connection* nc, int ev, void* ev_data)
	{
		// Only handle http requests
		if (ev != MG_EV_HTTP_REQUEST) return;
...
// 		else
	...
	void Http::FileHandler(mg_connection *nc, int ev, void *ev_data)
	{
		// Only handle http requests
		if (ev != MG_EV_HTTP_REQUEST) return;
		...
		//if (!Http::VerifyPassword(nc, message)) return;
...
// 		else
	...
	void Http::InfoHandler(mg_connection* nc, int ev, void* /*ev_data*/)
	{
		// Only handle http requests
		if (ev != MG_EV_HTTP_REQUEST) return;
	...
	void Http::EventHandler(mg_connection *nc, int ev, void *ev_data)
	{
		// Only handle http requests
		if (ev != MG_EV_HTTP_REQUEST) return;
...
// 				"Connection: close\r\n"
// 				"\r\n"
// 				"Hi fella!<br>You are%s connected to this server!", (Http::IsClient(nc) ? " " : " not"));
//
//				Game::client_t* client = Http::GetClient(nc);
//
// 			if (client)
// 			{
...
#pragma endregion
	...
	Http::Download()
	{
		...
		{
			ZeroMemory(&Http::Mgr, sizeof Http::Mgr);
			mg_mgr_init(&Http::Mgr, nullptr);

			Network::OnStart([]()
			{
				mg_connection* nc = mg_bind(&Http::Mgr, Utils::String::VA("%hu", Network::GetPort()), Http::EventHandler);

				if (nc)
				{
					// Handle special requests
					mg_register_http_endpoint(nc, "/info", Http::InfoHandler);
					mg_register_http_endpoint(nc, "/list", Http::ListHandler);
					mg_register_http_endpoint(nc, "/map", Http::MapHandler);
					mg_register_http_endpoint(nc, "/file/", Http::FileHandler);
					mg_register_http_endpoint(nc, "/serverlist", Http::ServerlistHandler);
            ...
            Http::ServerRunning = true;
			Http::Terminate = false;
			Http::ServerThread = std::thread([]
			{
				while (!Http::Terminate)
				{
					mg_mgr_poll(&Http::Mgr, 100);
				}
			});
		}
		else
		...
		Scheduler::OnFrame([]()
		{
			...
			for (auto i = Http::ScriptDownloads.begin(); i != Http::ScriptDownloads.end();)
			{
				auto download = *i;
				...
				{
					download->notifyDone();
					i = Http::ScriptDownloads.erase(i);
					continue;
				}
			...
			for (auto& download : Http::ScriptDownloads)
			{
		...
		Script::AddFunction("HttpGet", []
		{
			...
			Http::ScriptDownloads.push_back(std::make_shared<ScriptDownload>(url, object));
		...
		Script::AddFunction("HttpCancel", []
		{
			...
			for (const auto& download : Http::ScriptDownloads)
			{
	...
	Http::~Download()
	{
		if (Http::ServerRunning)
		{
			mg_mgr_free(&Http::Mgr);
		}
	}

	void Http::preDestroy()
	{
		Http::Terminate = true;
		if (Http::ServerThread.joinable())
		{
			Http::ServerThread.join();
		}
